...
@file:DependsOn("com.github.jknack:handlebars:4.5.1")
...
// without complex classpath setup. For the script, we'll keep the logic, but it's now
// identical to the testable class.
...
class LucideGenerator(
    ...
    fun generate() {
        ...
        val jsonFiles = sourceDir.listFiles { file -> file.extension == "json" }?.associateBy { it.nameWithoutExtension } ?: emptyMap()
    ...
    fun parseIconMetadata(jsonContent: String): IconMetadata {
        return try {
            ...
            val categories = element["categories"]?.jsonArray?.map { it.jsonPrimitive.content } ?: emptyList()
    ...
    private fun generateIconFile(svgFile: File, iconName: String) {
        ...
        val pathsCode = pathsData.joinToString("\n            ") { path -> 
            val indentedPath = convertPathDataToCompose(path).lines().joinToString("\n                ")
        ...
        val content = template.apply(mapOf(
            "name" to iconName,
            "paths" to pathsCode
        ))
        ...
        for (i in 0 until nodes.length) {
            ...
            if (node is Element) {
                when (node.tagName) {
                    ...
                    "rect" -> {
                        val x = node.getAttribute("x").let { if (it.isEmpty()) 0f else it.toFloat() }
                        val y = node.getAttribute("y").let { if (it.isEmpty()) 0f else it.toFloat() }
                        val w = node.getAttribute("width").toFloat()
                        val h = node.getAttribute("height").toFloat()
                        val rx = node.getAttribute("rx").let { if (it.isEmpty()) 0f else it.toFloat() }
                        val ry = node.getAttribute("ry").let { if (it.isEmpty()) 0f else it.toFloat() }
                        if (rx > 0 || ry > 0) {
    ...
    fun convertPathDataToCompose(d: String): String {
        ...
        while (i < tokens.size) {
            ...
            try {
                when (currentCommand) {
                    "M" -> { sb.append("moveTo(${tokens[i]}f, ${tokens[i+1]}f)\n"); i += 2 }
                    "m" -> { sb.append("moveToRelative(${tokens[i]}f, ${tokens[i+1]}f)\n"); i += 2 }
                    "L" -> { sb.append("lineTo(${tokens[i]}f, ${tokens[i+1]}f)\n"); i += 2 }
                    "l" -> { sb.append("lineToRelative(${tokens[i]}f, ${tokens[i+1]}f)\n"); i += 2 }
                    "H" -> { sb.append("horizontalLineTo(${tokens[i]}f)\n"); i += 1 }
                    "h" -> { sb.append("horizontalLineToRelative(${tokens[i]}f)\n"); i += 1 }
                    "V" -> { sb.append("verticalLineTo(${tokens[i]}f)\n"); i += 1 }
                    "v" -> { sb.append("verticalLineToRelative(${tokens[i]}f)\n"); i += 1 }
                    "C" -> { sb.append("curveTo(${tokens[i]}f, ${tokens[i+1]}f, ${tokens[i+2]}f, ${tokens[i+3]}f, ${tokens[i+4]}f, ${tokens[i+5]}f)\n"); i += 6 }
                    "c" -> { sb.append("curveToRelative(${tokens[i]}f, ${tokens[i+1]}f, ${tokens[i+2]}f, ${tokens[i+3]}f, ${tokens[i+4]}f, ${tokens[i+5]}f)\n"); i += 6 }
                    "S" -> { sb.append("reflectiveCurveTo(${tokens[i]}f, ${tokens[i+1]}f, ${tokens[i+2]}f, ${tokens[i+3]}f)\n"); i += 4 }
                    "s" -> { sb.append("reflectiveCurveToRelative(${tokens[i]}f, ${tokens[i+1]}f, ${tokens[i+2]}f, ${tokens[i+3]}f)\n"); i += 4 }
                    "Q" -> { sb.append("quadTo(${tokens[i]}f, ${tokens[i+1]}f, ${tokens[i+2]}f, ${tokens[i+3]}f)\n"); i += 4 }
                    "q" -> { sb.append("quadToRelative(${tokens[i]}f, ${tokens[i+1]}f, ${tokens[i+2]}f, ${tokens[i+3]}f)\n"); i += 4 }
                    "T" -> { sb.append("reflectiveQuadTo(${tokens[i]}f, ${tokens[i+1]}f)\n"); i += 2 }
                    "t" -> { sb.append("reflectiveQuadToRelative(${tokens[i]}f, ${tokens[i+1]}f)\n"); i += 2 }
                    "A" -> { 
            ...
            if (i < tokens.size && !tokens[i][0].isLetter() && currentCommand.lowercase() != "z") {
                 if (currentCommand.lowercase() == "m") currentCommand = if (currentCommand == "m") "l" else "L"
            } else {
                currentCommand = ""
            }
        }
    ...
    private fun generateMainObject(categoryNames: List<String>) {
        ...
        val categories = categoryNames.map { mapOf("name" to it.toPascalCase(), "className" to it.toPascalCase()) }
    ...
    private fun generateMetadata(icons: List<Map<String, Any>>) {
        ...
        chunks.forEachIndexed { index, chunk ->
            val content = chunkTemplate.apply(mapOf(
                "index" to index,
                "icons" to chunk
            ))
